...
| 🪟 **悬浮窗交互增强** | 滚轮滚动 / 边缘缩放 | 滚轮改为滚动释义内容（不再切词）；窗口四周可拖拽缩放，右下角带缩放手柄；并修复气泡关闭、设置加载等问题 |
| 🪟 **悬浮窗双模式** | 单词 / 知识 一键切换 | 悬浮窗顶部新增「单词 / 知识」分页签，可在**单词卡片**与 **AI 知识条目**之间自由切换；知识模式内嵌知识学习面板（含股票等领域），流式渲染、可追问，窗口尺寸按视图分别记忆 |
...
| 🛡 **免打扰** | 真实全屏检测 | 前台窗口真正全屏（如全屏看视频 / PPT 放映 / 游戏）时自动暂停推送；普通最大化窗口不触发 |
...
- 悬浮窗顶部「单词 / 知识」分页签，点击或按 `Tab` 键在两种展示形态间切换；单词视图保留「跳过 / 记住 / 打开」等操作栏，知识视图内嵌 AI 知识面板并自动隐藏操作栏
...
# 完整 NSIS 安装包
npm run build
# 产物：release/WordsFish Setup 1.1.0.exe
```
...
本仓库 v1.1.0 暂未启用自动更新，留作后续 milestone。
...
悬浮窗支持「单词」与「知识」两种展示形态，切换方式：

- 点击悬浮窗顶部分页签「单词 / 知识」
- 在悬浮窗内按 **`Tab`** 键快速来回切换

切换后窗口尺寸会按当前视图分别记忆（单词视图偏小、知识视图偏大），下次唤起时自动恢复上次所处的视图与尺寸；关闭悬浮窗（双击 / 快捷键）后仍会记住上次视图。
...
> 注：悬浮窗内的鼠标滚轮已改为滚动释义区域，不再切换单词；设置中的 `gestures.wheelUp` / `gestures.wheelDown` 配置项保留兼容，但悬浮窗默认以滚动为主。
...
- **会话持久化**：每个领域的对话历史存于 `userData/knowledge.json`，重启不丢；可一键清空，也可在会话列表里逐条删除（🗑）单个会话。
- **全局快捷键**：`Shift+K` 呼出独立知识窗口（托盘菜单也有入口）。
- **悬浮窗内嵌**：在悬浮窗顶部分页签切到「知识」，即可在悬浮窗内直接浏览 / 追问知识条目（股票、编程等领域均支持），无需另开大窗口；切换视图时窗口尺寸按「单词 / 知识」分别记忆。
...
2. 是否处于**真实全屏**（前台窗口铺满整屏，如全屏看视频 / PPT 放映 / 游戏；普通最大化窗口不会触发暂停）
3. 设置中是否勾了「暂停推送」
4. 托盘右键 → 「立即推送」是否能手动触发
...
- 🟠 修复**知识历史无限膨胀**：上下文窗口限制 20 条、持久上限 60 条滚动裁剪、会话保留最近 50 条，防 `knowledge.json` 越攒越大
- 🟠 修复**请求失败留下悬空消息**：LLM 调用失败时不再把用户提问落盘，避免知识面板出现「问了没答」的残影
- 🟠 修复**气泡按钮冻住**：气泡「跳过 / 记住」按钮不再误清自动关闭定时器，点击后正常推进下一词并保留自动关闭
- 🟠 修复**托盘双击冲突**：移除托盘 `double-click` 事件（仅保留单击切换悬浮窗），避免与系统托盘默认行为打架
- 🟠 修复**释义被意外重置**：悬浮窗收到配置热更新时不再重新加载单词，保留用户手动隐藏释义的状态
- 🟡 修复 9 处低优先级问题：开机自启开关状态不同步、悬浮窗尺寸缺省值缺失、学习统计缺 `exposed` 字段、选词引用相等误判、代理 CONNECT 超时无处理、流式错误响应体被丢弃、知识会话无法删除、托盘菜单误导性非全局快捷键文案、读路径产生配置写副作用
- 📄 新增 [`代码审计报告.md`](代码审计报告.md)：记录本次 16 项缺陷（1 高 / 6 中 / 9 低）的详细清单与修复说明

### v1.2.3（2026-08-04）
- 🪟 **悬浮窗「知识」分页签精简布局**：之前是 Shift+K 大窗口的完整页面（侧栏 + 4 个按钮 + 模式切换），在小悬浮窗里太挤
  - 移除侧栏（领域选择 / 会话列表）
  - 顶部只留「下一个知识点」按钮（去掉「清空会话」「配置接口」）
  - 去掉底部模式切换；改为**输入框联动**：空 = 生成知识卡片，输入文字 = 提问
  - 在消息流上方插入一行紧凑可滚动的**领域 chip 横排**，保证领域切换仍有入口
- 🛡 知识页面 `data-host="popup|standalone"` 区分宿主，Shift+K 大窗口不受影响（保留完整布局）
- 🧹 知识页面 `bindUI` / `setComposeDisabled` / `flash` 加 null 容忍，popup 模式下被移除的按钮不再抛错
...
- ✨ 悬浮窗内按 **`Tab`** 键可在「单词 / 知识」间快速切换；视图与窗口尺寸分别记忆（单词偏小、知识偏大），关闭后仍保留上次视图
- 🐛 知识视图自动隐藏自带标题栏的「最小化 / 关闭」按钮，避免误作用于悬浮窗宿主
- 🧪 测试加固：`cleanCustomDicts` 改为改名隔离以适配带 safe-delete 的沙箱；全部测试 50 + 33 + 23 通过

### v1.1.0（2026-08-04）
- ✨ 悬浮窗**可拖拽边缘缩放**：无边框窗口加 `thickFrame`，鼠标移到任意边缘即可拖拽改变大小；右下角新增缩放手柄（小三角）也可拖拽
- ✨ 悬浮窗**滚轮不再切词**：滚轮改为滚动释义内容，长释义也能顺畅浏览
- 🐛 托盘气泡新增 **✕ 关闭按钮**，可手动关闭；首次启动不再强制弹气泡，改为引导打开设置
- 🐛 修复设置界面 `data-cfg` 选择器导致的 `settings.js` 报错（null.checked）
- 🐛 打包加固：本地镜像绕过代理拉取 Electron 运行时，修复 `ffmpeg.dll` 缺失等问题
- 🧹 清理构建 / 临时产物，完善 `.gitignore`

### v1.0.0（2026-08-03）
- 🎉 首个正式版：托盘气泡推送、全局快捷键悬浮窗、SM2+ 间隔重复、多词库、5 套主题、AI 知识学习模块、开机自启等
