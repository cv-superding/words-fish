@echo off
REM 摸鱼背单词 - 启动器
REM 自动选择最新构建：优先 packout4（含滚轮兜底 + 知识 tab），其次 packout3 / packout2 / packout / release / dist
REM 启动前自动从 packout4 补齐所选构建目录可能缺失的关键运行时 DLL（ffmpeg.dll 等），
REM 彻底避免 Windows 加载器弹"找不到 xxx.dll"而 electron 还没起来就直接挂掉。
set "BASE=%~dp0"
set "GOLD=%BASE%packout4\win-unpacked"
...
REM 兜底：如果黄金构建 packout4 存在，先确保它自己 DLL 齐全（防御性）
call :ensure_dlls "%GOLD%"

REM 按优先级选构建并启动
for %%P in (packout4 packout3 packout2 packout release dist) do (
