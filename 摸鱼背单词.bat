...
REM 启动前自动从 packout3 补齐所选构建目录可能缺失的关键运行时 DLL（ffmpeg.dll 等），
REM 彻底避免 Windows 加载器弹"找不到 xxx.dll"而 electron 还没起来就直接挂掉。
set "BASE=%~dp0"
set "GOLD=%BASE%packout3\win-unpacked"
...
REM 兜底：如果黄金构建 packout3 存在，先确保它自己 DLL 齐全（防御性）
call :ensure_dlls "%GOLD%"

REM 按优先级选构建并启动
for %%P in (packout3 packout2 packout release dist) do (
